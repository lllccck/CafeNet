...
class CafeNet(nn.Module):
    ...
    def forward(self, image):
        ...
        p_again1 = F.interpolate(p2, size=mca2.size()[2:], mode='bilinear')
        enhence1 = mfe2 + mfe1
        enhence1 = p_again1.expand(-1, enhence1.size()[1], -1, -1).mul(enhence1)

        add_out2 = mca2 + enhence1
        ...
        p_again2 = F.interpolate(p3, size=mca1.size()[2:], mode='bilinear')
        enhence2 = mfe3 + mfe2 + mfe1
        enhence2 = p_again2.expand(-1, enhence2.size()[1], -1, -1).mul(enhence2)

        add_out3 = mca1 + enhence2
