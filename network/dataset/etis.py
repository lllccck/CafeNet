...
def LoadETISDataset(config):
    data = []
    image_dir = config["dataset_dir"] + "/ETIS-LaribPolypDB/images/*"
    ...
    def __init__(self, config, data, mode="train", normalization=True, augmentation=False):
        ...
        for index in np.arange(len(self.data)):
            ...
            if self.mode == "train" or self.mode == "val" or self.mode == "test":
                self.image_paths.append(config["dataset_dir"] + "/ETIS-LaribPolypDB/images/" + d["image"])
                self.label_paths.append(config["dataset_dir"] + "/ETIS-LaribPolypDB/masks/" + d["image"])
